...
import com.bitgam.app.ui.OnTaskCompleted;
...
public class AvatarWorkerTask extends AsyncTask<AvatarService.Avatarable, Void, Bitmap> {
    ...
    public void setOnTaskCompletedList(OnTaskCompleted a) {
        onTaskCompleted = a;
    }
    ...
    public static void loadAvatar(final AvatarService.Avatarable avatarable, final ImageView imageView, final @DimenRes int size, OnTaskCompleted a) {
        if (cancelPotentialWork(avatarable, imageView)) {
            final XmppActivity activity = XmppActivity.find(imageView);
            if (activity == null) {
                return;
